...
const ReportRow = ({
  ...
  <div
    className={cn(
      ...
      isTotal ? 'bg-slate-50 font-medium px-4 border-b border-black' : 'font-normal',
      isSubItem ? 'pl-8 pr-4' : 'px-2'
    )}
  >
    <div className="text-[10px] uppercase tracking-tight truncate leading-none">{label}</div>
    <div className="text-right font-mono text-[10px] font-normal flex items-baseline">
      ...
      <div className="a4-pdf-page p-[10mm] font-sans text-black bg-white w-[210mm] h-[297mm] flex flex-col box-border shadow-2xl relative overflow-hidden">
        <header className="flex justify-between items-start mb-6">
            ...
            <div className="flex flex-col items-center justify-center text-center flex-1 pt-4 px-4 overflow-hidden">
                ...
                <p className="font-medium text-[10px] uppercase tracking-wide mt-1" style={{ color: primaryGreen }}>Official P&L Statement</p>
            </div>
            <div className="text-right w-1/4 space-y-0.5">
                ...
                <div className="pt-3">
                    <Badge variant="outline" className="ml-auto text-[8px] font-normal uppercase py-0.5 border">AUDIT VALID</Badge>
                    <p className="text-[9px] font-normal text-muted-foreground mt-1">Period: {dateRange?.from ? format(dateRange.from, 'MMM yyyy') : 'Current'}</p>
                </div>
            </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 flex-grow">
            <div className="space-y-10">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-[9px] font-medium uppercase tracking-widest text-slate-500">1. Revenue (Inflow)</span>
                        <Badge variant="secondary" className="bg-blue-50 text-blue-800 border-none h-4 text-[8px] font-normal">GROSS SALES</Badge>
                    </div>
                ...
                <div className="bg-amber-50/30 p-4 rounded-xl border border-amber-100 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-[9px] font-medium uppercase tracking-widest text-amber-700">2. Direct Costs (COGS)</span>
                        <Badge variant="secondary" className="bg-amber-50 text-amber-800 border-none h-4 text-[8px] font-normal">ACQUISITION</Badge>
                    </div>
            ...
            <div className="space-y-10">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-[9px] font-medium uppercase tracking-widest text-slate-500">3. Overheads (Expenses)</span>
                        <Badge variant="secondary" className="bg-slate-100 text-slate-800 border-none h-4 text-[8px] font-normal">OPERATING SPEND</Badge>
                    </div>
                ...
                <div className={cn(
                    ...
                    <div className="flex justify-between items-center">
                        <span className="text-[9px] font-normal uppercase tracking-[0.2em] opacity-40">Period Performance</span>
                        <div className={cn("px-3 py-0.5 rounded-full", isNegative ? "bg-red-600 text-white" : "bg-emerald-600 text-white")}>
                            <span className="text-[8px] font-normal uppercase tracking-widest">{isNegative ? 'LOSS' : 'PROFIT'}</span>
                        </div>
                    </div>
                    <h2 className={cn("text-lg font-medium uppercase tracking-tight m-0", isNegative ? "text-red-700" : "text-emerald-700")}>
                        Net {isNegative ? 'Deficit' : 'Surplus'} Outcome
                    </h2>
                    <div className="pt-4 border-t border-black/5">
                        <span className={cn("text-2xl font-medium tracking-tight tabular-nums", isNegative ? "text-red-800" : "text-emerald-800")}>
        ...
        <footer className="mt-auto pt-6 border-t border-gray-100 flex justify-between items-end">
            <div className="text-left text-[8px] font-normal text-gray-400 uppercase tracking-widest">
                {companyName} &bull; Summarized Statement &bull; {format(new Date(), 'dd/MM/yy HH:mm')}
            </div>
            <div className="text-right">
                <p className="text-[9px] font-medium bg-gray-50 text-gray-400 px-4 py-1.5 rounded-sm uppercase">PAGE 1 OF {ledgerPages.length + 1}</p>
            </div>
        </footer>
      </div>
      ...
      {ledgerPages.map((pageData, pIdx) => (
          <div key={pIdx} className="a4-pdf-page p-[10mm] font-sans text-black bg-white w-[210mm] h-[297mm] flex flex-col box-border shadow-2xl relative overflow-hidden">
             <header className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
                <h3 className="text-lg font-medium uppercase tracking-tight m-0">Detailed Audit Ledger</h3>
                <div className="text-right font-normal uppercase text-[9px] opacity-30 tracking-widest">
                    {companyName}
                </div>
             </header>
                ...
                <table className="w-full border-collapse">
                    <thead className="bg-slate-50 text-slate-500 border-b border-slate-200">
                        <tr className="text-left">
                            <th className="p-3 font-medium text-[9px] uppercase w-24">Date</th>
                            <th className="p-3 font-medium text-[9px] uppercase">Reference & Description</th>
                            <th className="p-3 font-medium text-[9px] uppercase w-32">Type</th>
                            <th className="p-3 text-right font-medium text-[9px] uppercase w-40">Value (KES)</th>
                        </tr>
                    </thead>
                    <tbody>
                        {pageData.map((item: any, i) => {
                            ...
                            return (
                                <tr key={i} className="border-b border-gray-50 last:border-0">
                                    <td className="p-2 font-mono text-[9px] font-normal opacity-40">{format(parseISO(item.date), 'dd MMM yy')}</td>
                                    <td className="p-2">
                                        <p className="font-normal uppercase text-[9px] truncate max-w-[280px] tracking-tight">{item.label}</p>
                                        <p className="text-[7px] opacity-20 font-mono mt-0.5 uppercase">ID: {item.id.slice(0, 12)}</p>
                                    </td>
                                    <td className="p-2">
                                        <Badge variant="outline" className={cn("text-[7px] font-normal uppercase h-3.5 px-1.5 border-none", isIncome ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-800")}>
                                            {isIncome ? 'INFLOW' : 'OUTFLOW'}
                                        </Badge>
                                    </td>
                                    <td className={cn("p-2 text-right font-normal tabular-nums text-[9px] tracking-tight", isIncome ? "text-emerald-700" : "text-red-700")}>
                                        {formatCurrency(amount)}
                                    </td>
                                </tr>
                            );
             ...
             <footer className="mt-6 pt-6 border-t border-gray-100 flex justify-between items-center bg-white">
                <p className="text-[8px] font-normal text-gray-300 uppercase tracking-widest">{companyName} &bull; Detailed Statement</p>
                <p className="text-[9px] font-medium bg-gray-50 text-gray-300 px-4 py-1.5 rounded-sm tracking-widest">PAGE {pIdx + 2} OF {ledgerPages.length + 1}</p>
             </footer>
          </div>
      ))}
    </div>
