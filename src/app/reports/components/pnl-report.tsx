...
const ReportRow = ({
  ...
  <div
    className={cn(
      ...
      isTotal ? 'bg-slate-50 font-black px-4 border-b-2 border-black' : 'font-bold',
      isSubItem ? 'pl-8 pr-4' : 'px-2'
    )}
  >
    <div className="text-[10px] uppercase tracking-tight truncate leading-none">{label}</div>
    <div className="text-right font-mono text-[10px] font-black flex items-baseline">
      ...
      <div className="a4-pdf-page p-[10mm] font-sans text-black bg-white w-[210mm] h-[297mm] flex flex-col box-border shadow-2xl relative overflow-hidden">
        <header className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4 w-1/4">
              {company?.logoUrl ? (
                  ...
                  <div className="h-14 w-14 bg-gray-50 flex items-center justify-center text-[10px] font-black border-2 border-dashed border-gray-200 text-gray-300">LOGO</div>
              )}
            </div>
            <div className="flex flex-col items-center justify-center text-center flex-1 pt-4 px-4 overflow-hidden">
                <h1 className="text-[20px] font-black uppercase tracking-tight leading-none truncate w-full" style={{ color: primaryBlue }}>
                    {companyName}
                </h1>
                <p className="font-black text-[10px] uppercase tracking-wide mt-1" style={{ color: primaryGreen }}>Official P&L Statement</p>
            </div>
            <div className="text-right w-1/4 space-y-0.5">
                ...
                <div className="pt-3">
                    ...
                    <p className="text-[9px] font-black text-muted-foreground mt-1">Period: {dateRange?.from ? format(dateRange.from, 'MMM yyyy') : 'Current'}</p>
                </div>
            </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 flex-grow">
            <div className="space-y-10">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">1. Revenue (Inflow)</span>
                ...
                <div className="bg-amber-50/30 p-4 rounded-xl border border-amber-100 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-[9px] font-black uppercase tracking-widest text-amber-700">2. Direct Costs (COGS)</span>
            ...
            <div className="space-y-10">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 shadow-sm">
                    <div className="flex justify-between items-center mb-4">
                        <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">3. Overheads (Expenses)</span>
                ...
                <div className={cn(
                    ...
                    <div className="flex justify-between items-center">
                        <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40">Period Performance</span>
                    ...
                    <h2 className={cn("text-lg font-black uppercase tracking-tight m-0", isNegative ? "text-red-700" : "text-emerald-700")}>
        ...
        <footer className="mt-auto pt-6 border-t border-gray-100 flex justify-between items-end">
            ...
            <div className="text-right">
                <p className="text-[9px] font-black bg-gray-50 text-gray-400 px-4 py-1.5 rounded-sm uppercase">PAGE 1 OF {ledgerPages.length + 1}</p>
            </div>
        </footer>
      </div>
      ...
      {ledgerPages.map((pageData, pIdx) => (
          <div key={pIdx} className="a4-pdf-page p-[10mm] font-sans text-black bg-white w-[210mm] h-[297mm] flex flex-col box-border shadow-2xl relative overflow-hidden">
             <header className="flex justify-between items-center mb-6 pb-4 border-b border-slate-100">
                ...
                <div className="text-right font-black uppercase text-[9px] opacity-30 tracking-widest">
                    {companyName}
                </div>
             </header>
                ...
                <table className="w-full border-collapse">
                    <thead className="bg-slate-50 text-slate-500 border-b border-slate-200">
                        <tr className="text-left">
                            ...
                            <th className="p-3 font-black text-[9px] uppercase w-32">Type</th>
                            <th className="p-3 text-right font-black text-[9px] uppercase w-40">Value (KES)</th>
                        </tr>
                    </thead>
                    <tbody>
                        {pageData.map((item: any, i) => {
                            ...
                            return (
                                <tr key={i} className="border-b border-gray-50 last:border-0">
                                    <td className="p-2 font-mono text-[9px] font-black opacity-40">{format(parseISO(item.date), 'dd MMM yy')}</td>
                                    <td className="p-2">
                                        <p className="font-black uppercase text-[9px] truncate max-w-[280px] tracking-tight">{item.label}</p>
                                        <p className="text-[7px] opacity-20 font-mono mt-0.5 uppercase">ID: {item.id.slice(0, 12)}</p>
                                    </td>
                                    <td className="p-2">
                                        <Badge variant="outline" className={cn("text-[7px] font-black uppercase h-3.5 px-1.5 border-none", isIncome ? "bg-emerald-50 text-emerald-800" : "bg-red-50 text-red-800")}>
                                            {isIncome ? 'INFLOW' : 'OUTFLOW'}
                                        </Badge>
                                    </td>
                                    <td className={cn("p-2 text-right font-black tabular-nums text-[9px] tracking-tight", isIncome ? "text-emerald-700" : "text-red-700")}>
                                        {formatCurrency(amount)}
                                    </td>
                                </tr>
                            );
             ...
             <footer className="mt-6 pt-6 border-t border-gray-100 flex justify-between items-center bg-white">
                <p className="text-[8px] font-black text-gray-300 uppercase tracking-widest">{companyName} &bull; Detailed Statement</p>
                <p className="text-[9px] font-black bg-gray-50 text-gray-300 px-4 py-1.5 rounded-sm tracking-widest">PAGE {pIdx + 2} OF {ledgerPages.length + 1}</p>
             </footer>
          </div>
      ))}
    </div>
