...
export function ChatClient() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Team Chat"
        description="Instant communication for your shop staff."
      />
      <Card className="shadow-xl h-[70vh] flex flex-col border-none ring-1 ring-border bg-white">
        <CardHeader className="border-b bg-muted/10">
          <div className="flex items-center justify-between">
            <div>
                ...
                <CardDescription className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Talk with your team instantly.</CardDescription>
            </div>
            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 animate-pulse h-6">
                <Zap className="h-3 w-3 mr-1 fill-green-700" /> Secure Area
            </Badge>
          </div>
        </CardHeader>
        <CardContent className="flex-grow overflow-hidden bg-card/50">
          <ScrollArea className="h-full pr-4" ref={scrollAreaRef}>
            <div className="space-y-6 py-6">
              {isLoading && <p className="text-center text-muted-foreground animate-pulse text-xs font-bold uppercase tracking-widest">Checking messages...</p>}
                ...
                return (
                    <div key={message.id} className={cn(
                      ...
                      {!isMe && !isPlatformAdmin && (
                        <Avatar className="h-9 w-9 ring-2 ring-background border shadow-sm">
                            <AvatarImage src={message.userAvatar} alt={message.userName} />
                            <AvatarFallback className="bg-primary/5 text-primary text-xs font-bold">{message.userName.substring(0,2).toUpperCase()}</AvatarFallback>
                        </Avatar>
                      )}
                      ...
                      {isPlatformAdmin ? (
                        ...
                        <div className={cn(
                            ...
                            isMe ? 'bg-primary text-primary-foreground rounded-tr-none' : 'bg-white rounded-tl-none border'
                        )}>
                       ...
                       {isMe && !isPlatformAdmin && (
                        <Avatar className="h-9 w-9 ring-2 ring-background border shadow-sm">
                            <AvatarImage src={message.userAvatar} alt={message.userName} />
                            <AvatarFallback className="bg-primary text-white text-xs font-bold">{message.userName.substring(0,2).toUpperCase()}</AvatarFallback>
                        </Avatar>
                      )}
                    </div>
                );
        ...
        <CardFooter className="pt-4 border-t bg-muted/5">
          <form onSubmit={handleSendMessage} className="flex w-full items-center space-x-2">
            <Input
              ...
              className="flex-1 h-12 rounded-xl bg-white border-none ring-1 ring-border shadow-inner px-4 font-medium"
              autoComplete="off"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              disabled={!user || isLoading}
            />
            <Button type="submit" size="icon" className="h-12 w-12 rounded-xl shadow-lg transition-transform active:scale-95 bg-primary text-white" disabled={!user || isLoading || !input.trim()}>
