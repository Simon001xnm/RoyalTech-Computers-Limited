 ...
 * Matches the requested indigo layout: Logo Right, Title Left, words for totals.
 */
    ...
    <div className="p-[15mm] font-sans text-[12px] bg-white text-black w-[210mm] min-h-[297mm] flex flex-col box-border selection:bg-indigo-100">
      ...
      <header className="flex justify-between items-start mb-8">
        <div className="space-y-4">
            <h1 className="text-4xl font-medium tracking-tight" style={{ color: primaryIndigo }}>Receipt</h1>
            <div className="space-y-1 text-[13px] font-medium text-black">
                <p><span className="w-28 inline-block opacity-60">Receipt No</span> <span className="font-bold">{workspace?.receiptPrefix || 'RCT'}{receiptNo}</span></p>
                <p><span className="w-28 inline-block opacity-60">Receipt Date</span> <span className="font-bold">{format(new Date(docSnapshot.generatedDate), "MMM dd, yyyy")}</span></p>
            </div>
        </div>
                ...
                {items?.map((item: SaleItem, idx: number) => {
                    ...
                    return (
                        <tr key={idx} className="border-b border-gray-100">
                            ...
                            <td className="py-4 text-right text-[11px] font-medium">KES {formatCurrency(item.price)}</td>
        ...
        <div className="flex justify-between items-start mt-8">
            <div className="max-w-[400px]">
                <p className="text-[11px] font-bold text-black uppercase">
                    Total (in words) : {numberToWords(amount)}
                </p>
            </div>
            ...
            <div className="w-[320px] space-y-4">
                <div className="flex justify-between items-center text-[11px]">
                    ...
                    <span className="font-bold">KES {formatCurrency(subtotal || amount)}</span>
                </div>
                <div className="flex justify-between items-center text-[11px]">
                ...
                <div className="pt-4 border-t-2 border-black flex justify-between items-center">
                    ...
                    <span className="text-[18px] font-bold">KES {formatCurrency(amount)}</span>
                </div>
