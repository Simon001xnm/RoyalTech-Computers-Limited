  ...
  return (
    <div className="p-[20mm] font-sans text-[12px] bg-white text-black w-[210mm] min-h-[297mm] flex flex-col box-border border border-gray-100 selection:bg-black selection:text-white">
      ...
      <header className="flex justify-between items-start mb-12">
        <div className="flex items-center gap-6">
          ...
          <div className="space-y-1">
             <h1 className="text-2xl font-black uppercase leading-tight tracking-tighter">{companyName}</h1>
        ...
        <div className="text-right">
            ...
            <div className="space-y-1 text-[11px] font-bold">
                <p><span className="opacity-60 uppercase">Receipt No:</span> {workspace?.receiptPrefix || 'RCT'}-{receiptNo}</p>
      ...
      <section className="mb-12">
        <h3 className="font-black uppercase text-[11px] border-b-2 border-black pb-1 w-fit mb-3">RECEIVED FROM:</h3>
      ...
      <section className="flex-grow">
        <table className="w-full border-collapse">
            <thead>
                <tr className="border-b-2 border-black text-left">
                    <th className="py-3 font-black uppercase text-[11px]">ITEM DESCRIPTION</th>
                ...
                {items?.map((item: SaleItem, idx: number) => (
                    <tr key={idx} className="font-bold border-b border-black border-dotted">
                        <td className="py-4 align-top pr-4">
                            <p className="leading-tight text-sm uppercase">{item.name}</p>
                        </td>
        ...
        <div className="flex justify-between items-start mt-10">
            <div className="space-y-4">
                <div>
                    <h4 className="font-black uppercase text-[10px] border-b border-black pb-0.5 mb-2">PAYMENT MODE</h4>
                    <p className="font-black text-sm uppercase">{paymentMethod}</p>
                    {referenceCode && <p className="font-mono text-[10px] mt-1">REF: {referenceCode}</p>}
                </div>
            </div>
            <div className="w-[350px] space-y-2">
                <div className="flex justify-between items-center py-1 border-b border-black/10">
                    ...
                    <span className="font-bold text-sm">KES {formatCurrency(subtotal || amount)}</span>
                </div>
                {applyVat && (
                    <div className="flex justify-between items-center py-1 border-b border-black/10">
                ...
                {changeDue > 0 && (
                    <div className="flex justify-between items-center py-2 px-4 italic bg-gray-100 border border-black/5">
        ...
        <div className="mt-16 text-center">
            <div className="inline-block border-[4px] border-black border-double px-12 py-4 mb-6">
                <h2 className="text-3xl font-black uppercase tracking-tighter">PAID IN FULL</h2>
            </div>
            <p className="text-lg font-black uppercase">THANK YOU FOR YOUR BUSINESS</p>
        </div>
      </section>

      {/* COMPLIANCE FOOTER */}
      <footer className="mt-auto pt-10 border-t-2 border-black">
         <div className="text-[10px] flex justify-between items-end font-bold">
            <div className="space-y-1">
            ...
            <div className="text-right space-y-1">
                ...
                <p className="opacity-40 uppercase text-[8px]">Receipt ID: {docSnapshot.id}</p>
            </div>
         </div>
      </footer>
    </div>
